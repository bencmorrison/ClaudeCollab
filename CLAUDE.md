...
- **The thin-pointer SHAPE is machine-checked again; the SEMANTICS are not.** `modelguild/tests/check-claude-md.sh` (CI) restores the structural check the retired bash `doctor.sh` had: the `@AGENTS.md` first line, regular-file-not-symlink, the 60-line anti-fork ceiling, and no literal restatement of `AGENTS.md`'s shared rules (it greps for a few distinctive markers). What the lint **cannot** see is a shared rule paraphrased in fresh words so no marker fires — deciding what counts as Claude-specific versus a restatement of `AGENTS.md` is still judgment, held by hand. Keep this file a thin pointer that adds only what `AGENTS.md` does not say.
