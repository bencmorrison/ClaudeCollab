...
- [x] **Own the read-only agent permissions — DONE (2026-07-15).** Built `collab-read` (`.opencode/agent/collab-read.md`) denying `bash`/`write`/`edit`/`patch`; ask.sh now defaults to it (was `plan`). *The built-in `plan` agent does not hard-deny `bash`; its safety was model compliance, not construction. `collab-read` is construction.*
  ...
  - **Still open:** `collab-build` (deny secret files + external dirs + egress for the `--edit`/`/delegate` path — the review flagged the asymmetry: reads are locked down, writes are still `allow *` under `--auto`) — not yet built.
...
- **False read-only safety** → *addressed for the read path:* `collab-read` hard-denies mutation **and secret reads and network egress** by construction (secret denies as a `read:` object-map because opencode is last-match-wins), verified by `collab/verify-collab-read.sh`; re-run it on opencode upgrades. The "read-only ≠ safe" hole (readable `.env` + open `webfetch`) was found in expert review and fixed. *Still to do:* wire verify into CI/doctor to catch silent schema-drift automatically, and give the `--edit` path the same treatment (`collab-build`).
