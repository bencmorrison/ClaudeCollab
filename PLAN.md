...
- [ ] Run the **static** agent verification in CI (`verify-collab-*.sh --static`) to catch permission-schema drift automatically — e.g. someone re-adding `"*": allow` or deleting the deny floor. Needs `opencode` installed in CI (npm i -g opencode-ai); `opencode agent list` must resolve the config **without auth** — verify that works headless before wiring it in (untested). Until then, `doctor.sh` covers this locally. (Review finding: per-commit CI currently proves syntax/tests but not resolved permissions.)
