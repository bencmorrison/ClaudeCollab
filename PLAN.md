...
- **False read-only safety** → *addressed for the read path:* `collab-read` hard-denies mutation (`bash`/`edit`/`write`/`patch`/`task`/`todowrite`) **and secret reads and network egress** by construction (secret denies as a `read:` object-map because opencode is last-match-wins), verified by `collab/verify-collab-read.sh`; re-run it on opencode upgrades. The "read-only ≠ safe" hole (readable `.env` + open `webfetch`) and a later `patch` mutation gap were both found and fixed. *The `--edit` path (`collab-build`) is now hardened too* — denies `task`/`webfetch`/`websearch` + secret reads, but allows `bash` by choice (needed for builds/tests), so its secret/egress denies are defense-in-depth, not by construction; the diff review is the boundary (verified by `collab/verify-collab-build.sh`). *Done (2026-07-15):* both verify scripts' `--static` checks run in `collab/doctor.sh` and CI, catching silent permission-schema drift automatically on every push.
