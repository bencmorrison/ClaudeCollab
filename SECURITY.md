...
- The `collab_delegate` tool **snapshots your worktree** before the write agent runs (a git tree, via a throwaway index — your files and staged state are untouched) and records the model's complete diff at `collab/logs/<run_id>/diff-<call_id>.patch` (returned as `structuredContent.capture.patchPath`). Two consequences worth knowing: the patch is the model's changes **only**, even if you had uncommitted work in progress; and anything of yours the model overwrites is **recoverable** (`git checkout <tree> -- <path>`, in `capture.recoveryHint`). There is deliberately **no clean-worktree requirement** — delegating onto live work is the point, and a snapshot serves it better than a refusal did.
...
- **Model policy.** Policy rules are first-match globs with default-allow. Resolution is `$COLLAB_POLICY` when set, otherwise a `collab/models.policy.local` containing at least one rule, otherwise committed `collab/models.policy`; an empty/comment-only local file does not mask the shared policy. The tools refuse a `deny` model and require explicit user approval (`confirmed:true`) for an `ask` model. Fail-closed on a malformed or unreadable policy file.
- **Preflight.** `npx claudecollab doctor` checks tools, auth, the MCP registration, the policy, and the payload before you rely on the commands. The resolved-config permission proofs (`verify-collab-*.sh`) are run locally after an opencode bump.
...
- **These are receipts** — the other model's actual words, on disk and yours to read, so you can check Claude's account of an exchange against what was really said. It is git-ignored, never transmitted, and pruned after 14 days (`COLLAB_LOG_RETENTION_DAYS`). It is a *local file*, not a network surface — but it is a plaintext copy of repo context living outside the files it came from, and it will be caught by anything that archives or backs up your working tree.
- **Set `COLLAB_LOG_PROMPTS=hash`** (digest only — proves the prompt didn't change, reveals none of it) **or `off`**, in `collab/collab.conf.local`, if that copy isn't acceptable for your work. `COLLAB_LOG=off` disables the log entirely.
