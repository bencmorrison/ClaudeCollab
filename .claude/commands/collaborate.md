...
allowed-tools: Bash(bash collab/ask.sh:*), Bash(COLLAB_CONFIRMED=1 bash collab/ask.sh:*), Bash(opencode models:*), Bash(COLLAB_COMMAND=/collaborate bash collab/ask.sh:*), Bash(COLLAB_COMMAND=/collaborate COLLAB_CONFIRMED=1 bash collab/ask.sh:*), Bash(COLLAB_RUN_ID=* COLLAB_COMMAND=/collaborate bash collab/ask.sh:*), Bash(bash collab/log.sh:*)
---
...
6. **One targeted rebuttal — only if it changes the decision.** If a disagreement materially matters, send ONE follow-up on the same session:
   `COLLAB_RUN_ID=$RUN COLLAB_COMMAND=/collaborate bash collab/ask.sh --emit-session -s <id> -m <provider/model> "<your specific pushback or question>"`
