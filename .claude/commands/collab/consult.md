...
1. Run `COLLAB_COMMAND=/collab:consult bash collab/ask.sh "<restate the question with enough context from our conversation that a fresh model can answer standalone>"`. This uses our `collab-read` agent, which denies file mutation, secret reads, and network egress at opencode's permission layer (read-only by construction — see AGENTS.md). If the agent def is missing it falls back to opencode's weaker `plan` agent (read-only by compliance only), never to a write-capable agent.
   ...
   - **Model policy:** check `collab/models.policy` before choosing. Never use a model matched by a `deny` rule; if your choice matches an `ask` rule, confirm with the user first, then invoke as `COLLAB_COMMAND=/collab:consult COLLAB_CONFIRMED=1 bash collab/ask.sh …`. Always state the exact `provider/model` id you used.
