...
allowed-tools: mcp__claudecollab__collab_consult, mcp__claudecollab__collab_models, Task
---
...
This uses `collab_consult` with **session continuation** (`keepSession` / `sessionId`), so the other model keeps its own memory of the exchange in opencode and **you never re-transmit its words** — that keeps its side of the record faithful by construction, not by your discipline.
