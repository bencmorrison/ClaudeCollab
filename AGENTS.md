...
- **`collab/ask.sh`** — the single entry point. Wraps `opencode run --agent <agent> --auto [-m provider/model] <prompt>`. Also supports `-s/--session <id>` (continue an opencode session), `--emit-session` (print `SESSION: <id>` + the extracted answer, for `/collaborate`'s multi-turn Option B), and `--dry-run` (print the exact opencode command, safely quoted, and exit without calling a model — token-free, the policy backstop still applies). Run `bash collab/ask.sh -h` for its interface. **Note:** it redirects opencode's stdin from `/dev/null` — load-bearing, see Gotchas.
  - **`collab-read` agent (default, read-only) = `.opencode/agent/collab-read.md`** — denies `bash`/`write`/`edit`/`patch` at opencode's permission layer, so the mutating tools are **removed from the model's toolset**, not merely discouraged. Read-only *by construction*, not by model compliance. `--auto` cannot approve a denied tool into existence. Proven by **`collab/verify-collab-read.sh`** (adversarial: tells the agent to write a file with auto-approve on, then asserts the file was NOT created — filesystem, not the model's self-report). Run that after any bump of opencode or the agent def. **Must stay `mode: all`** — a `mode: subagent` agent invoked via `opencode run --agent` silently falls back to the full-access `build` agent (the verify script guards this).
