...
- `collab-research` → the source-backed `/collab:research` workflow agent: **same allow-set as `collab-read`** (`read` + `grep` + `glob` + web); mutation and `task` denied. Same posture — **not a confidentiality boundary, trusted repos only**: it can read any file including secrets and reach the web by design. Verified by `collab/verify-collab-research.sh`.
...
- **`/collab:research` is the source-backed web workflow.** Same read + web exposure and same "trusted repos only — it can read your secrets and reach the web" posture; its value is the command workflow requiring citations and Claude verification. Proven by `collab/verify-collab-research.sh`.
