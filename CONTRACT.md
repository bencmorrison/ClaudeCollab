...
- **C15** — Default agent is `guild-read`. `--edit` → `guild-build`; `--research` → `guild-research`; `--watch` → `guild-watch`. `-a <agent>` overrides; an unrecognized agent runs as-is with a stderr note.
- **C16** — Fallback when a hardened def is missing: `guild-read` → `plan` (loud warning); `guild-build` → `build` (loud warning); `guild-research` → `plan` (never `build`, loud warning). Presence is checked in `$agent_def_dir` (`$GUILD_AGENT_DIR` > `conf_get` > sibling `.opencode/agent`), which governs **only** the fallback decision, not opencode's own `--agent` resolution.
- **C17** — `GUILD_REQUIRE_HARDENED=1` turns any such fallback into a hard error, exit **5**, no call.
...
- **C57** — `5` required-hardened def missing (always for `--watch`, incl. bad watch log-dir scope); `8` `--watch` model unset, or `--watch` on a Claude model without `GUILD_CONFIRMED=1`.
