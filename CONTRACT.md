...
- **C48** — Per-agent allow-sets: `collab-read` = read non-secret + webfetch/websearch; `collab-build` = edit/write/patch/bash; `collab-research` = read non-secret + webfetch/websearch; `collab-watch` = **read `collab/logs/**` only** (inverted map, all else incl. secrets denied). Secret-path globs are denied under `read` where applicable.
- **C49** — Honest-guarantee statements are part of the contract: `collab-read`/`collab-research` are **not** exfiltration or credential boundaries (read + web reachable; secret globs are a list, not a floor); `collab-build`'s non-mutation denies are **defense-in-depth, not by construction** (`bash` routes around them) — the human diff review is the boundary; `collab-watch`'s `bash`/`grep`/`glob`/egress denials are load-bearing.
- **C50** — `grep`/`glob` are denied on read paths as a **named harness difference** (opencode's grep returns secret content walking with `--hidden`), not a permanent parity principle. MCP tools are covered by the `"*": deny` floor.
